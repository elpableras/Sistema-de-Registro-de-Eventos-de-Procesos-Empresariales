 ...
 */
public class Generador extends JDialog {

	/**
	 * Serial.
	 */
	private static final long serialVersionUID = -355153293351333296L;
	private JScrollPane panelListaGeneradores = null;
	private DefaultListModel modeloLista = null;
	private JList listaGene = null;

	private JTextField tFGenerador;

	private JLabel lblGenerador = null;
	 ...
	 * 
	 * @param lista
	 *            Lista con las personas generadoras.
	 * @param p
	 *            Objeto de tipo Procedimiento1y2, que será donde se devuelva la
	 *            lista creada.
	 */
	public Generador(List<Object> lista, Procedimiento1y2 p) {
		super();
		this.lista = lista;
		this.pg = p;
		initialize();
	...
	private void initialize() {
		...
		panelGene.setBorder(new TitledBorder(new EtchedBorder(
				EtchedBorder.LOWERED, null, null), "Lista de Generadores",
				TitledBorder.LEADING, TitledBorder.TOP, null,
				new Color(0, 0, 0)));
		getContentPane().add(panelGene);
		panelGene.setLayout(null);

		panelGene.add(getPanelListaGeneradores(), null);
		rellenarLista();
	...
	/**
	 * Método que inicializa el panel de scroll de la lista de generadores.
	 * 
	 * @return javax.swing.JScrollPane.
	 */
	private JScrollPane getPanelListaGeneradores() {
		if (panelListaGeneradores == null) {
			panelListaGeneradores = new JScrollPane();
			panelListaGeneradores.setToolTipText("");
			panelListaGeneradores.setBounds(new Rectangle(10, 121, 336, 170));
			panelListaGeneradores.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.LOWERED, null, null), "Lista de Generadores", TitledBorder.LEADING, TitledBorder.ABOVE_TOP, null, new Color(51, 51, 51)));
			panelListaGeneradores.setViewportView(getListaVisitas());
		}
		return panelListaGeneradores;
	}

	/**
	 * Método que inicializa el JList de la lista de generadores.
	 * 
	...
	private JList getListaVisitas() {
		...
		if (listaGene == null) {
			listaGene = new JList(modeloLista);
		}
		return listaGene;
	}

	/**
	 * Método para rellenar la lista de generadores.
	 */
	...
	/**
	 * Método que inicializa la etiqueta del Código de generadores a aplicar.
	 * 
	 * @return javax.swing.JLabel.
	 */
	private JLabel getLabelCod() {
		if (lblGenerador == null) {
			lblGenerador = new JLabel("Generadores a aplicar:");
			lblGenerador.setBounds(26, 34, 254, 14);
		}
		return lblGenerador;
	}

	/**
	 * Método que inicializa el TextField de Generador.
	 * 
	...
	private JTextField getTFGenerador() {
		if (tFGenerador == null) {
			...
			tFGenerador.setToolTipText("Generador");
	...
	private JButton getBtEliminar() {
		if (btEliminar == null) {
			...
			btEliminar.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					modeloLista.removeElement(listaGene.getSelectedValue());
					cont--;
					if (cont < 2) {
						btEliminar.setEnabled(false);
					}
		...
		if (btnAceptar == null) {
			...
			btnAceptar.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					// LISTA DE GENERADOR GENERAL
					pg.setListaGene(lista);
	...
	private void setAyuda() {
		...
		try {
			...
			// Ayuda para botón
			hb.enableHelpOnButton(btnAyuda, "generadores", hs);
			// Desplega ayuda pulsando sobre F1
			hb.enableHelpKey(this.getContentPane(), "generadores", hs);
		} catch (Exception e) {
